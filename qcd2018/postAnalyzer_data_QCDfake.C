...
void postAnalyzer::BookHistos(const char* file2)
{
  ...
  for(int i=0; i<10; i++)
  {
    ...
    for(int j=0; j<9; j++)
    {
      ...
      h_phoPt_num_un[i][j] = new TH1F(("h_phoPt_num_un"+histname_i+histname_j).c_str(),"h_phoPt_num_un",165,0.,1000.);h_phoPt_num_un[i][j]->Sumw2();
      ...
      h_phoPt_num_QCD_un[i][j] = new TH1F(("h_phoPt_num_QCD_un"+histname_i+histname_j).c_str(),"h_phoPt_num_QCD_un",165,0.,1000.);h_phoPt_num_QCD_un[i][j]->Sumw2();
...
void postAnalyzer::fillHistosNum(int sysNumber, int index,int event_weight)
{
  ...
  h_phoPt_num[histoNumber][sysNumber]->Fill(uncorrectedPhoEt,event_weight); //d0412 corrected
  ...
  if(histoNumber > 0)
  {
    ...
    h_phoPt_num_un[histoNumber][sysNumber]->Fill(uncorrectedPhoEt);
    h_pfMET_num[histoNumber][sysNumber]->Fill(pfMET,event_weight);
  }
  if(nVtx <= 11)
    histoNumber = 6;
