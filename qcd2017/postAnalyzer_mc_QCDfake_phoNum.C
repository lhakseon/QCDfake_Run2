...
//  cout<<"start@#!#!$!#!#!@#@!"<<endl;  
    ...
    for(int p = 0; p < genPhotons.size(); p++)
    {
      ...
      if(phoCandDen.size() > 0)
        dRtemp = dR(mcEta->at(genPhotons[p]),mcPhi->at(genPhotons[p]),phoSCEta->at(phoCandDen[0]),phoSCPhi->at(phoCandDen[0])); //den
      if(dRtemp < dRmin_Den)
        dRmin_Den = dRtemp;
      dRtemp = 9999.9;
     ...
     if(metFilters==0)
     {
       if(HLTPho>>11&1 == 1)
       {
        if(pfMET < 30)
        {
          ...
          {
            event_weight = 1.0;
            event_weight *= sieie_correction((*phoSigmaIEtaIEtaFull5x5)[phoCandDen[0]], 0);
            ...
            event_weight = 1.0;
            event_weight *= sieie_correction((*phoSigmaIEtaIEtaFull5x5)[phoCandDen[0]], 1);
              fillHistosDen(7,phoCandDen[0],event_weight);
            event_weight = 1.0;
            event_weight *= sieie_correction((*phoSigmaIEtaIEtaFull5x5)[phoCandDen[0]], 2);
